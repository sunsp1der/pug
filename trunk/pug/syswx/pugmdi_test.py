...
#        sizer = wx.BoxSizer()
#        sizer.Add(self.GetClientWindow(), 1, wx.EXPAND)
#        self.SetSizer(sizer)
...
class PugMDIChild(  wx.Panel, PugFrame):
...
#        aui.AuiMDIChildFrame.__init__(self, parent, -1, '', name=name,
#                                         **kwargs)
 #       self.Bind(wx.EVT_ACTIVATE, self._evt_on_activate)
 #       self.Bind(wx.EVT_CLOSE, self._evt_on_close)
 #       self.Bind(wx.EVT_MENU, self._evt_passmenu)        
#        wx.GetApp().set_default_pos( self)
        win = self.setup_window(obj, objectpath, title, name)
        self.GetSizer().Add( win.menuBar)
        parent._mgr.AddPane( self, aui.AuiPaneInfo().CenterPane())
