...
"""
        ...
        oldscene_class = self.Director.scene.__class__            
        if oldscene_class != value:
            close_scene_windows(oldscene)
            self.Director.scene = value
            # wait for completion
            starttime = time.time()
            while self.Director.scene.__class__ != value:
    ...
    def revert_scene(self):
        ...
        close_scene_windows()
        sceneclass = self.Director.scene = self.Director.scene.__class__
    ...
    def save_scene(self, sceneName=None):
...
"""
        if not sceneName:
            name = self.Director.scene.gname
            if not name:
            ...
            while not sceneName:
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    return
            dlg.Destroy()
        path = os.path.join('scenes',''.join([sceneName,'.py']))  
        self.Director.scene.state = None          
        wx.BeginBusyCursor()
...
#        Opioid2D.Director.scene.__class__ = loadscene
        if loadscene:
            self.Director.scene.__class__ = loadscene
            #hack
            sceneFrame = wx.GetApp().get_object_pugframe( self.Director.scene)
            if sceneFrame:
...
"""
    ...
    def start_scene(self):
        # self.save_scene( self.get_sceneclass().__name__)
...
_interfaceTemplate = {
    ...
    'attributes':[ 
        ...
        ['save_scene', None,  {'label':'   Save Scene', 'use_defaults':True}],
